...
def add_worker(request):
    ...
    if request.method == "POST" and request.is_ajax:
        ...
        if user_form.is_valid() and worker_form.is_valid():
            ...
            return JsonResponse({"success":True, "worker_html": worker_html}, status=200)
        username = request.POST['username']
...
def change_worker(request):
    ...
    if request.method == "POST" and request.is_ajax:
        ...
        if worker_form.is_valid():
            ...
            if worker.user.username == request.user.username:
                return JsonResponse({"success":True, "worker_html": worker_html, "worker": worker.id, 'avatar': worker.avatar.url}, status=200)
...
def add_task(request):
    ...
    if request.method == "POST" and request.is_ajax:
        available_from = datetime.strptime(request.POST['available_from'], '%d.%m.%Y').date()
        available_to = datetime.strptime(request.POST['available_to'], '%d.%m.%Y').date()
        if available_from > available_to:
            return JsonResponse({"success":False, "message":"Invalid date from ... to!"}, status=200)
...
def end_task(request):
    ...
    if request.method == "POST" and request.is_ajax:
        ...
        worker = get_object_or_404(Worker, id=worker_key)
        task.end()
        start_timelog = TimeLog.objects.filter(task=task, worker=worker, action="start working")
        timelog = TimeLog.objects.create(task=task, worker=worker, action="finish working", comment=None)
        if start_timelog:
            old_timelog = start_timelog[0]
            add_time = time.strftime("%H:%M:%S", time.gmtime((timelog.time_of_start - old_timelog.time_of_start).total_seconds()))
            new_time = (datetime.strptime(task.spent_time, '%H:%M:%S') + timedelta(seconds=(timelog.time_of_start - old_timelog.time_of_start).total_seconds())).strftime("%H:%M:%S")
            task.spent_time = new_time
            task.save()
            timelog.spent_time = add_time
            timelog.save()
            return JsonResponse({"success":True, 'time': new_time}, status=200)
        timelog.save()
        return JsonResponse({"success":True}, status=200)
    return JsonResponse({"success":False}, status=400)
...
def active_tasks(request):
    ...
    if request.method == "GET" and request.is_ajax:
        tasks_list = Task.objects.filter(is_active=True, is_completed=False)
        worker = get_object_or_404(Worker, user=request.user)
        if tasks_list:
            tasks_html = loader.render_to_string(
                'Tracker/content.html',
                {'tasks_list': tasks_list, 'worker': worker, 'user': request.user}
            )
            return JsonResponse({"success":True, "tasks_html": tasks_html}, status=200)
        else:
            return JsonResponse({"success":False}, status=200)
    return JsonResponse({"success":False}, status=400)
...
def get_messages(request):
    ...
    if request.method == "GET" and request.is_ajax:
        worker_key = request.GET['worker']
        worker = get_object_or_404(Worker, id=worker_key)
        messages = Message.objects.filter(receiver=worker)[:15]
        if messages:
